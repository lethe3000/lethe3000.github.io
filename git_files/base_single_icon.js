...
//            if (testLangs.indexOf('|' + similarproducts.b.userData.lang + '|') == -1 || similarproducts.utilities.abTestUtil.getBucket() != 'uiCoreLocalized_A') {    //language not in test
            if (similarproducts.languages && similarproducts.languages[similarproducts.b.userData.lang] && similarproducts.utilities.abTestUtil.getBucket() == '2014w11_uiCoreLocalizedv2_A') {
            }    //language in test
            else {
			...
			if (!spsupport.sites.isBlackStage()) {
				...
				if (similarproducts.utilities.abTestUtil.getBucket() == '2014w11_Redesign05_A' || similarproducts.utilities.abTestUtil.getBucket() == '2014w11_Redesign05_B' || similarproducts.utilities.abTestUtil.getBucket() == 'uiRedesign01LT_A')
				{
...
//            spsupport.log("before cpn wl request: site type = " + sp.siteType + ";  pageType = " + sp.pageType);
//            spsupport.log(sp.prodPage.p);
//            spsupport.log(sp.prodPage);
//            spsupport.log(sp.SRP);
		...
		addSimilarProductsSupport: function(){
			...
			if( !top.similarproducts.util && !spsupport.p.iSf)
			{
				...
				if (similarproducts.utilities.abTestUtil.getBucket() == 'uiRedesign01LT_A')
				{
		...
		startDOMEnumeration: function()
		{
			...
			if( ss.validRefState() )
			{
				...
				if (similarproducts.utilities.abTestUtil.getBucket() == '2014w10_ImageValidator_A')
				{
					similarproducts.imageValidator.init(sp.$);

					images = similarproducts.imageValidator.enumerate(images, false);
				}
		...
		positionSFDiv: function(img, area)
		{
			...
			else if (similarproducts.utilities.abTestUtil.getBucket() == '2014w09_Redesign04_B')
			{
				...
				{
					left: ipos.x,
					top: ipos.y+height - button.height(),
					width: width
				});
			}
			else
